...
const detailsSlideAnim = React.useRef(new Animated.Value(initialWidth * 1.2)).current;
...
export default function MainScreen({ onLogout }: MainScreenProps) {
  ...
  const colors = {
    ...
    button: theme === 'dark' ? '#ff8b4c' : '#ff8b4c',
    toggleSwitch: theme === 'dark' ? '#404040' : '#ddd',
    toggleActive: theme === 'dark' ? '#ff8b4c' : '#ff8b4c',
  };

  // Отладка - ПРОВЕРКА ОБНОВЛЕНИЯ КОДА
  console.log('MainScreen rendered - theme:', theme, 'language:', language, 'UPDATED:', new Date().toLocaleTimeString());
  ...
  const detailsSlideAnim = React.useRef(new Animated.Value(initialWidth * 1.2)).current;
  const slideAnim = React.useRef(new Animated.Value(-initialWidth)).current;
  ...
  const toggleMenu = () => {
    const toValue = isMenuOpen ? -initialWidth : 0;
  ...
  const getCurrentData = useMemo(() => () => {
    ...
    // Фильтрация по расстоянию (стабильная логика без рандома)
    return allData;
  }, [activeTab]);

  const getMarkerColor = useMemo(() => () => '#ff8b4c', []);

  const getMarkerEmoji = useMemo(() => () => {
    switch (activeTab) {
      case 'walks':
        return '🐕';
      case 'places':
  ...
  const closeDetailsPanel = () => {
    Animated.timing(detailsSlideAnim, {
      toValue: initialWidth * 1.2, // Уходим за правый край
      duration: 300,
      useNativeDriver: false,
    }).start(() => {
  ...
  const TabButton = ({ type, title }: { type: TabType; title: string }) => (
    <TouchableOpacity
      style={[styles.tabButton, activeTab === type && { backgroundColor: colors.tabActive }]}
      onPress={() => {
    ...
    >
      {isTabLoading && activeTab === type ? (
        ...
        <Text style={[styles.tabButtonText, activeTab === type && { color: colors.text }]}>
          {title}
        </Text>
      )}
    </TouchableOpacity>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <View style={{...styles.header, backgroundColor: colors.header, borderBottomColor: colors.border}}>
        ...
        {/* Tab Switcher */}
        <View style={{...styles.tabSwitcher, backgroundColor: colors.tabSwitcher}}>
            ...
            {sampleAds.map((ad) => (
              <View key={ad.id} style={[styles.adCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <View style={styles.adCardContent}>
                  ...
                  <Text style={[styles.adDescription, { color: colors.textSecondary }]}>{ad.description}</Text>
                </View>
              </View>
            ))}
          </ScrollView>
      ...
      {/* Footer */}
      <View style={[styles.footer, { backgroundColor: colors.header, borderTopColor: colors.border }]}>
        <View style={styles.adBanner}>
          <Text style={[styles.adBannerText, { color: colors.textSecondary }]}>Реклама: Корм премиум-класса для активных питомцев</Text>
        </View>
      </View>
      ...
      {/* Settings Menu */}
      {isSettingsMenuOpen && (
        <View style={[styles.sidePanel, { right: 0, backgroundColor: colors.card}]}>
          <View style={styles.sidePanelHeader}>
            <View style={styles.sidePanelTitleContainer}>
          ...
          <ScrollView style={styles.panelList}>
            <TouchableOpacity style={styles.panelItem} onPress={() => setNotifications(!notifications)}>
              <View style={styles.panelItemContent}>
                <Text style={styles.panelItemTitle}>Уведомления</Text>
                <Text style={styles.panelItemSubtitle}>{notifications ? 'Включены' : 'Выключены'}</Text>
              </View>
              <View style={[styles.toggleSwitch, notifications && styles.toggleSwitchActive]}>
            ...
            <TouchableOpacity style={styles.panelItem} onPress={() => setShowLanguageMenu(true)}>
              <View style={styles.panelItemContent}>
                ...
                <Text style={[styles.panelItemSubtitle, { color: colors.textSecondary }]}>{language === 'ru' ? 'Русский' : 'English'}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  overlay: {
    ...
    zIndex: 1000, // Над контентом но под меню
  },
  sidePanel: {
    position: 'absolute',
    top: 0,
    right: -320, // Скрыто за пределами экрана
    width: 280, // Фиксированная ширина
    height: '100%',
    backgroundColor: '#fff',
    zIndex: 1001, // Над оверлеем
    shadowColor: '#000',
  ...
  detailsPanel: {
    ...
    bottom: 0,
    backgroundColor: '#fff',
    paddingTop: initialWidth < 400 ? 20 : 30,
    paddingHorizontal: initialWidth < 400 ? 12 : 16,
    zIndex: 3,
    shadowColor: '#000',
    shadowOffset: { width: 2, height: 0 },
    shadowOpacity: 0.2,
    shadowRadius: 8,
    elevation: 8,
    maxWidth: initialWidth < 400 ? 300 : 350,
  ...
  detailsTitle: {
    fontSize: initialWidth < 400 ? 16 : 18,
    fontWeight: '700',
    flex: 1,
    textAlign: 'center',
    marginHorizontal: 8,
  ...
  detailsLabel: {
    fontSize: 13,
    fontWeight: '700', // Жирнее для пунктов меню
    color: '#888',
    marginTop: 8,
    marginBottom: 8,
    paddingBottom: 8,
    borderBottomWidth: 1,
    borderBottomColor: 'rgba(0,0,0,0.1)', // Тонкая разделительная линия
  },
  detailsValue: {
    fontSize: 15,
    color: '#333',
