...
// Плейсхолдеры для нативной карты (react-native-maps временно отключён,
// чтобы не ломать web-сборку). На web/нативе сейчас используется FallbackMap.
...
export default function MainScreen({ onLogout }: MainScreenProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Side Menu */}
      {isMenuOpen && (
        <View style={[styles.sidePanel, { transform: [{ translateX: 0 }] }]}>
          ...
          <ScrollView style={styles.panelList}>
            ...
            <TouchableOpacity style={styles.panelItem} onPress={openMyEvents}>
              <View style={styles.panelItemContent}>
                <Text style={styles.panelItemTitle}>Мои события</Text>
                <Text style={styles.panelItemSubtitle}>Список запланированных мероприятий</Text>
              </View>
            </TouchableOpacity>
